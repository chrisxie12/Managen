...
/**
 * OAuth start endpoint - initiate Google OAuth flow
 */
app.post("/oauth/start", async (req, res) => {
  try {
    ...
    startCallbackServer(state, 120000)
      .then(async (code) => {
        try {
          ...
          logger.success(`[OAuth] Successfully added account: ${result.email}`);
        } catch (error) {
          logger.error("[OAuth] Flow completion failed:", error.message);
          oauthSessions.set(state, {
            ...oauthSessions.get(state),
            status: "failed",
            error: error.message,
      ...
      .catch((error) => {
        logger.error("[OAuth] Callback server error:", error.message);
        oauthSessions.set(state, {
          ...oauthSessions.get(state),
          status: "failed",
          error: error.message,
  ...
  try {
    ...
    res.json({
      accounts: accounts.map((acc) => ({
        ...
        sessionKey: acc.refreshToken
          ? acc.refreshToken.substring(0, 20) + "..."
          : null,
      })),
    });
  } catch (error) {
