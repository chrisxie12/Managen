...
const S = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .lp-section-label { display: inline-block; font-size: 11px; font-weight: 700; letter-spacing: 0.14em; text-transform: uppercase; color: #0B4F30; background: rgba(11,79,48,0.08); padding: 4px 12px; border-radius: 100px; margin-bottom: 16px; }
  ...
  .lp-plan-feature { display: flex; gap: 8px; font-size: 13px; color: #374151; line-height: 1.4; }
  .lp-plan-feature::before { content: "✓"; color: #0B4F30; font-weight: 700; flex-shrink: 0; }
  .lp-plan.featured .lp-plan-feature { color: rgba(255,255,255,0.75); }
  .lp-plan.featured .lp-plan-feature::before { color: #D4FF00; }
  ...
  .lp-trust-quote { max-width: 720px; margin: 0 auto; text-align: center; padding: 40px; background: #fff; border: 1px solid #E5E7EB; border-radius: 20px; }
  .lp-trust-quote-text { font-size: 18px; font-style: italic; color: #0B0F1C; line-height: 1.7; margin-bottom: 20px; }
  ...
  .lp-footer-logomark { width: 34px; height: 34px; background: #D4FF00; border-radius: 9px; display: flex; align-items: center; justify-content: center; font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 17px; color: #0B0F1C; flex-shrink: 0; }
  .lp-footer-brandname { font-family: 'Space Grotesk', sans-serif; font-weight: 700; font-size: 19px; color: #fff; }
  .lp-footer-tagline { font-size: 14px; color: rgba(255,255,255,0.42); line-height: 1.6; max-width: 240px; }
  ...
  .lp-footer-love { font-size: 13px; color: rgba(255,255,255,0.3); }
    ...
    <div style={{ padding: 10 }}>
      ...
      {rows.map((r) => (
        <div key={r.name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "5px 6px", borderRadius: 6, background: "#F9FAFB", marginBottom: 3 }}>
          ...
          <span style={{ fontSize: 7, fontWeight: 700, padding: "2px 6px", borderRadius: 100, background: r.present ? "rgba(11,79,48,0.12)" : "rgba(239,68,68,0.1)", color: r.present ? "#0B4F30" : "#EF4444" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5 }}>
        {metrics.map((m) => (
          <div key={m.label} style={{ background: "#F9FAFB", borderRadius: 8, padding: 7 }}>
            ...
            <div style={{ fontSize: 7, color: m.up ? "#0B4F30" : "#EF4444" }}>{m.trend}</div>
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{ fontFamily: "'DM Sans', system-ui, sans-serif", background: "#fff" }}>
      ...
      <section className="lp-section" id="pricing" style={{ background: "#fff" }}>
        ...
        <div className="lp-pricing-grid">
          {PLANS.map((plan) => (
            <div key={plan.tier} className={`lp-plan${plan.featured ? " featured" : ""}`}>
              ...
              <ul className="lp-plan-features">
                {plan.features.map((f) => (
                  <li key={f} className="lp-plan-feature">{f}</li>
                ))}
              </ul>
              <button
                className={`lp-plan-btn ${plan.btnClass}`}
      ...
      <section className="lp-section" style={{ background: "#F5F5F4" }}>
        ...
        <div className="lp-calc-wrap">
          <div className="lp-calc-row">
            <div className="lp-calc-field">
              <label>How many students?</label>
              <select value={calcStudents} onChange={(e) => setCalcStudents(Number(e.target.value))}>
            ...
            <div className="lp-calc-field">
              <label>How many campuses?</label>
              <select value={calcCampuses} onChange={(e) => setCalcCampuses(Number(e.target.value))}>
      ...
      <section className="lp-section" id="features" style={{ background: "#fff" }}>
        ...
        <div className="lp-hiw-cards">
          {FEATURES.map((feat, i) => (
            <div key={feat.eyebrow} className="lp-hiw-card">
              <div className={`lp-hiw-inner${feat.rev ? " rev" : ""}`}>
                ...
                <div className="lp-hiw-phone-wrap">
        ...
        <div className="lp-trust-grid">
          {TRUST_LOGOS.map((logo, i) => (
            <div key={i} className="lp-trust-logo">
              <div className="lp-trust-icon" style={{ background: logo.bg, color: logo.color }}>{logo.abbr}</div>
              <div className="lp-trust-name">{logo.name}</div>
            </div>
          ))}
        </div>
        <div className="lp-trust-quote">
          <p className="lp-trust-quote-text">
            "Finally, a school management system that understands how Ghanaian schools actually work. The WhatsApp integration alone saves me two hours every Monday."
          </p>
          ...
          <div className="lp-footer-grid">
            <div>
              <a className="lp-footer-brand-row" href="/">
                <div className="lp-footer-logomark">M</div>
