...
import {
  ...
  Filter, Users, Clock, TrendingDown, Loader2,
} from "lucide-react";
...
interface OutstandingStudent {
  id: string; name: string; admNo: string; class: string;
  total: number; paid: number; outstanding: number; daysSince: number;
}
...
export function BursarOutstanding() {
  ...
  const classes = ["All Classes", ...Array.from(new Set(students.map(s => s.class))).filter(Boolean).sort()];
  ...
  async function sendBulkReminders() {
    ...
    try {
      ...
      setReminderLoading(prev => { const n = new Set(prev); ids.forEach(id => n.delete(id)); return n; });
  ...
  const summaryCards = [
    ...
    { label: "Students w/ Balance", value: students.length.toString(), icon: <Users className="w-5 h-5" />, color: NAVY, bg: "#EFF6FF" },
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm mt-1" style={{ color: MUTED }}>Track and manage unpaid student fees</p>
      </div>
      ...
      <div className="bg-card border border-border rounded-2xl p-5">
        ...
        <div className="flex flex-wrap gap-3">
          <select className="border border-border rounded-xl px-3 py-2 text-sm focus:outline-none bg-white"
      ...
      <div className="bg-card border border-border rounded-2xl p-5">
        ...
        {loading ? (
          ...
          <div className="text-center py-12" style={{ color: MUTED }}>
            {students.length === 0 ? "No outstanding balances found." : "No students match the selected filters."}
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  ...
                  {["Student Name", "Adm No", "Class", "Total Fees", "Paid", "Outstanding", "Days", "Status", "Actions"].map((h) => (
                    ...
                    <td className="py-3 px-3">
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          onClick={() => navigate(`/dashboard/bursar/collect?student=${s.id}`)}>
                        ...
                        {reminderSent.has(s.id) ? (
                          ...
                          <button
                            ...
                            style={{ color: NAVY }}
                            disabled={reminderLoading.has(s.id)}
        ...
        <div className="flex items-center gap-4 mt-4 pt-4 border-t border-border flex-wrap">
          ...
          {[
            ...
            <span key={item.label} className="text-xs px-2 py-0.5 rounded-full font-medium"
