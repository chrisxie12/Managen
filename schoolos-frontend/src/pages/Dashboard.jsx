...
const Dashboard = React.memo(({ onNavigate }) => {
  ...
  return (
    <div className="p-6 sm:p-8 flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {metrics.map((card) => (
          ...
          >
            ...
            <div style={{ color: "var(--text-muted)", fontSize: "0.75rem", marginBottom: "0.4rem", fontWeight: 700 }} className="relative z-10 uppercase tracking-widest">
              {card.label}
            </div>
            <div className="flex items-center gap-1.5 text-xs font-black relative z-10" style={{ color: card.positive ? "#10B981" : "#EF4444" }}>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 p-8 rounded-[32px] border" style={{ background: "var(--card-bg)", borderColor: "var(--border)", boxShadow: "var(--shadow-card)" }}>
          <div className="flex items-center justify-between mb-8">
            <div>
              <h2 style={{ fontFamily: "'Playfair Display', serif", color: "var(--text-primary)", fontWeight: 800, fontSize: "1.1rem" }}>Fee Collection Flow</h2>
              <p style={{ color: "var(--text-muted)", fontSize: "0.75rem", fontWeight: 600 }}>Monthly revenue intake vs projected</p>
            </div>
            <div className="flex gap-2">
               <button onClick={() => onNavigate("fees")} className="px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest border hover:bg-black/5 transition-colors" style={{ color: "var(--text-primary)", borderColor: "var(--border)" }}>Ledger Details</button>
            </div>
          </div>
          <ResponsiveContainer width="100%" height={250}>
            <AreaChart data={revenueData}>
              ...
              <XAxis dataKey="month" tick={{ fontSize: 12, fill: "var(--text-muted)", fontWeight: 700 }} axisLine={false} tickLine={false} />
        ...
        <div className="p-8 rounded-[32px] border flex flex-col" style={{ background: "var(--card-bg)", borderColor: "var(--border)", boxShadow: "var(--shadow-card)" }}>
          <h2 style={{ fontFamily: "'Playfair Display', serif", color: "var(--text-primary)", fontWeight: 800, fontSize: "1.1rem", marginBottom: "1.5rem" }}>Financial Status</h2>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 p-8 rounded-[32px] border" style={{ background: "var(--card-bg)", borderColor: "var(--border)", boxShadow: "var(--shadow-card)" }}>
          <h2 style={{ fontFamily: "'Playfair Display', serif", color: "var(--text-primary)", fontWeight: 800, fontSize: "1.1rem", marginBottom: "1.5rem" }}>System Feed</h2>
              ...
              <div key={i} className="flex items-start gap-4 group cursor-pointer">
                ...
                <div className="flex-1 border-b pb-4" style={{ borderColor: "var(--border)" }}>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                     <span style={{ color: "var(--text-muted)", fontSize: "0.75rem", fontWeight: 700 }}>{item.time}</span>
                     <span className="w-1 h-1 rounded-full bg-slate-200" />
                     <span style={{ color: "var(--text-muted)", fontSize: "0.75rem", fontWeight: 700, textTransform: "capitalize" }}>{item.type}</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        <div className="flex flex-col gap-6">
           <div className="p-8 rounded-[32px] shadow-xl relative overflow-hidden bg-plum" style={{ boxShadow: "var(--shadow-glow)" }}>
              ...
              <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.2rem", fontWeight: 800, marginBottom: "1.2rem", color: "rgba(255,255,255,0.95)" }}>Top Performers</h2>
                 ...
                 {topStudents.map((s, i) => (
                   <div key={s.id} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                         ...
                         <p className="text-xs font-bold opacity-80" style={{ color: "var(--bg-primary)" }}>{s.class} · {s.id}</p>
                      </div>
                      <div style={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 800, fontSize: "0.95rem", color: "var(--bg-primary)" }}>{s.avg}%</div>
                   </div>
                 ))}
              </div>
              <button onClick={() => onNavigate('results')} className="mt-6 w-full py-3 rounded-2xl bg-milk text-plum text-xs font-black uppercase tracking-widest hover:scale-105 active:scale-95 transition-all">View Full Ranking</button>
           </div>

           <div className="p-8 rounded-[32px] border flex flex-col gap-4" style={{ background: "var(--card-bg)", borderColor: "var(--border)" }}>
              <h2 style={{ fontFamily: "'Playfair Display', serif", color: "var(--text-primary)", fontWeight: 800, fontSize: "1rem" }}>Quick Hub</h2>
              <div className="grid grid-cols-2 gap-2">
                 {[
                   { label: "Add Student", icon: Plus, color: "#EC4899", path: "students" },
                   { label: "Reminders", icon: Zap, color: "#00e5a0", path: "fee-reminders" },
