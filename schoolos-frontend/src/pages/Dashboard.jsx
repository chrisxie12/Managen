      ...
      <main className="lg:ml-64 pt-16 p-6 sm:p-8 flex flex-col gap-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
           ...
           <div className="flex gap-2">
              <button className="p-2.5 rounded-xl border bg-white shadow-sm hover:bg-slate-50 transition-colors" title="Notifications" style={{ background: "var(--card-bg)", borderColor: "var(--border)" }}>
                 <Bell size={18} style={{ color: "var(--text-primary)" }} />
              </button>
              <button className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest shadow-lg shadow-plum/20 hover:scale-105 active:scale-95 transition-all" style={{ background: isDarkMode ? "#FFF3E6" : "#381932", color: isDarkMode ? "#381932" : "#FFF3E6" }}>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {metrics.map((card) => (
            ...
            >
              <div className="absolute top-0 right-0 w-16 h-16 bg-slate-50/50 rounded-bl-full -mr-8 -mt-8 group-hover:scale-125 transition-transform duration-500" />
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 p-8 rounded-[32px] border" style={{ background: "var(--card-bg)", borderColor: "var(--border)", boxShadow: "var(--shadow-card)" }}>
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex gap-2">
                 <button onClick={() => onNavigate("fees")} className="px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border hover:bg-slate-50 transition-colors" style={{ color: "var(--text-primary)", borderColor: "var(--border)" }}>Ledger Details</button>
              </div>
            </div>
              ...
              <AreaChart data={revenueData}>
                <defs>
                  <linearGradient id="revenueGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor={isDarkMode ? "#FFF3E6" : "#381932"} stopOpacity={0.2} />
                    <stop offset="95%" stopColor={isDarkMode ? "#FFF3E6" : "#381932"} stopOpacity={0.01} />
                  </linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke={isDarkMode ? "rgba(255,243,230,0.05)" : "rgba(56,25,50,0.04)"} vertical={false} />
                ...
                <Tooltip
                  contentStyle={{ background: "var(--card-bg)", border: `1px solid var(--border)`, borderRadius: 16, fontSize: 11, fontWeight: 700, boxShadow: "var(--shadow-card)" }}
                  formatter={(v) => [`₵${v.toLocaleString()}`, "Revenue"]}
                />
                <Area type="monotone" dataKey="amount" stroke={isDarkMode ? "#FFF3E6" : "#381932"} strokeWidth={3} fill="url(#revenueGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="flex flex-col gap-6">
             <div className="p-8 rounded-[32px] shadow-xl relative overflow-hidden" style={{ background: isDarkMode ? "#381932" : "linear-gradient(135deg, #381932 0%, #512b4a 100%)", boxShadow: "var(--shadow-glow)" }}>
                <Star className="absolute -right-6 -top-6 w-32 h-32 opacity-10 rotate-12" color="#FFF3E6" />
                <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: "1.2rem", fontWeight: 800, marginBottom: "1.2rem", color: "#FFF3E6" }}>Top Performers</h3>
                <div className="space-y-4">
                   {topStudents.map((s, i) => (
                     <div key={s.id} className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center font-black text-xs" style={{ background: i === 0 ? "#F59E0B" : "rgba(255,243,230,0.15)", color: "#FFF3E6" }}>
                           {i + 1}
                        </div>
                        <div className="flex-1 min-w-0">
                           <p className="text-sm font-bold truncate" style={{ color: "#FFF3E6" }}>{s.name}</p>
                           <p className="text-[10px] font-bold opacity-60" style={{ color: "#FFF3E6" }}>{s.class} · {s.id}</p>
                        </div>
                        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontWeight: 800, fontSize: "0.95rem", color: "#FFF3E6" }}>{s.avg}%</div>
                     </div>
                   ))}
                </div>
             ...
             <div className="p-8 rounded-[32px] border flex flex-col gap-4" style={{ background: "var(--card-bg)", borderColor: "var(--border)" }}>
                ...
                <div className="grid grid-cols-2 gap-2">
                   {[
                     ...
                     <button key={act.label} onClick={() => onNavigate(act.path)} className="p-3 rounded-2xl flex flex-col items-center gap-2 transition-all hover:bg-slate-50" style={{ background: `${act.color}08`, border: `1px solid ${act.color}15` }}>
